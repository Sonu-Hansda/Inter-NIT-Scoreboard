...
import { getTableTennisKnockout, getTableTennisPools } from '../../lib/api';
import type { TableTennisKnockoutRow, TableTennisPoolRow } from '../../lib/api';
import PastGameTable from './PastGameTable';
import TeamRow from './TeamRow';
import type { Game, Row } from '../../types/Game';
import { mapTableTennisKnockoutToGame, mapTableTennisPoolsToGames } from '../../lib/mappers';
import SkeletonTable from '../ui/SkeletonTable';

export default function TableTennisScoreboard() {
  const [pools, setPools] = useState<Record<string, TableTennisPoolRow[]>>({});
  const [knockout, setKnockout] = useState<TableTennisKnockoutRow[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    Promise.all([getTableTennisPools(), getTableTennisKnockout()])
  ...
  const poolGames = mapTableTennisPoolsToGames(pools);
  const knockoutGame = mapTableTennisKnockoutToGame(knockout);
  ...
  if (loading) {
    ...
    const poolHeaders = ['Team', 'Won', 'Loss', 'Points'];
